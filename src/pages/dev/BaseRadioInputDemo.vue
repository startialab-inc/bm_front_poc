...
<template>
  <div class="min-h-screen bg-slate-100 p-8 font-sans">
    ...
    <!-- セクション2: グループ選択（AppRadioButton Molecule） -->
    <section class="mb-6 bg-white rounded-lg p-6 shadow-sm">
      <h2 class="text-lg font-semibold text-slate-700 border-b border-slate-200 pb-2 mb-4">
        AppRadioButton（Molecule）でのグループ選択
      </h2>
      <p class="text-xs text-slate-400 mb-4">
        AppRadioButton は BaseRadioInput に <code class="bg-slate-100 px-1 rounded">label</code> を組み合わせた Molecule。
        同じ <code class="bg-slate-100 px-1 rounded">name</code> を指定すると相互排他になる。
      </p>
      <div class="flex items-center gap-4 flex-wrap mb-3">
        <AppRadioButton v-model="groupSelected" value="option1" name="group-demo" label="オプション 1" />
        <AppRadioButton v-model="groupSelected" value="option2" name="group-demo" label="オプション 2" />
        <AppRadioButton v-model="groupSelected" value="option3" name="group-demo" label="オプション 3" />
      </div>
